...
'''
        ...
        stage('Test') {
            steps {
                ...
                sh '''
                curl -f http://backend:5000/api || curl -f http://host.docker.internal:5000/api || echo "Backend not reachable"
                '''

                echo 'Testing frontend availability...'
                sh '''
                curl -f http://frontend:5173 || curl -f http://host.docker.internal:5173 || echo "Frontend not reachable"
                '''
            }
        }
    }
